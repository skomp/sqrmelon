...
char*	lpGMDLSSuffix = "\\drivers\\gm.dls";
...
void _64klang_Init(BYTE* songStream, void* patchData, DWORD const1Offset, DWORD const2Offset, DWORD maxOffset)
{
	...
	SynthMemCopy(lpGMDLSName+len, lpGMDLSSuffix, 30);
...
bool ProcessStream(int currentTimeStamp, int& streamIndex)
{
	// read info (1 DWORD)
...
void _64klang_Render(float* dstbuffer)
{
	...
	int maxTimeStamp = *((DWORD*)(SynthGlobalState.SongStream+4));
